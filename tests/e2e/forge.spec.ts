...
async function installWebMCPHarness(page: Page) {
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, '__forgeRegisteredTools', {
      configurable: true,
      value: registry,
    });
  });
...
test('landing enters the laboratory and registers every WebMCP capability', async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole('button', { name: 'Talk to Garrick' }),
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Talk to Elara' }),
  ).toBeVisible();
  await expect(
    page.getByRole('button', { name: 'Talk to Rowan' }),
  ).toBeVisible();
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          (
            window as unknown as {
              __forgeRegisteredTools: Map<string, unknown>;
            }
          ).__forgeRegisteredTools.size,
      ),
    )
    .toBe(30);
});

test('native proposal waits for approval, regression proves the repair, rollback restores it, and reset is repeatable', async ({
  page,
}) => {
  await page.goto('/lab');
  await expect
    ...
    .toBe(30);
  await expect(
    page.getByText(/Paste it into the Codex or ChatGPT conversation/),
  ).toBeVisible();

  const proposal = await page.evaluate(async () => {
    const registry = (
      window as unknown as {
        __forgeRegisteredTools: Map<
          string,
          {
            execute: (
              input: Record<string, unknown>,
    ...
    const response = await registry.get('modify_item_spawn')?.execute({
      ...
      reason:
        'Repair the circular progression dependency with the narrowest reversible change.',
    });
  ...
  await expect(
    page.getByRole('button', { name: /Agent repair ready/ }),
  ).toBeVisible();
  ...
  const keyRemainsBehindGate = await page.evaluate(async () => {
    ...
    const response = await registry
      .get('get_location_state')
      ?.execute({ location_id: 'loc-crypt-sanctum' });
    const result = JSON.parse(response?.content[0].text ?? '{}') as {
      data?: { items?: Array<{ id: string }> };
    };
    return (
      result.data?.items?.some((item) => item.id === 'item-crypt-key') ?? false
    );
  });
  expect(keyRemainsBehindGate).toBe(true);

  await page.getByRole('button', { name: 'Approve', exact: true }).click();
  await expect(page.getByText('Executed result')).toBeVisible();
  ...
  await expect(page.getByText(/World revision \d+/)).toBeVisible();
  await expect(
    page.getByText('Required key is locked behind its own gate'),
  ).toHaveCount(0);
  ...
  const resetRestoredDefect = await page.evaluate(async () => {
    ...
    const response = await registry
      .get('find_progression_blockers')
      ?.execute({});
    const result = JSON.parse(response?.content[0].text ?? '{}') as {
      data?: { blockerCount?: number };
    };
...
test('keyboard navigation exposes accessible controls', async ({ page }) => {
  ...
  await expect(
    page.getByRole('button', { name: 'Copy next prompt' }),
  ).toBeVisible();
});

test('narrow layout keeps the world atlas and agent activity available', async ({
  page,
}, testInfo) => {
  test.skip(
    testInfo.project.name !== 'narrow-chatgpt-pane',
    'Narrow-layout assertion runs in the narrow project.',
  );
  await page.goto('/lab');
  await expect
    .poll(() =>
  ...
  await expect(
    page.getByRole('navigation', { name: 'Locations' }),
  ).toBeVisible();
  await page.getByRole('button', { name: 'Agent activity' }).click();
  await expect(
    page.locator('aside[aria-label="Agent activity"]:visible'),
  ).toBeVisible();
});
