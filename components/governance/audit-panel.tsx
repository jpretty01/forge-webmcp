  ...
  const entries = useMemo(
    () =>
      state.auditLog.filter(
        (entry) =>
          (scope === 'all' || entry.runId === currentRunId) &&
          `${entry.toolName} ${entry.category} ${entry.summary}`
            .toLowerCase()
            .includes(filter.toLowerCase()),
      ),
    [currentRunId, filter, scope, state.auditLog],
  );
  if (state.auditLog.length === 0)
    return (
      <section className="empty-state">
        <Filter className="size-7 text-amber-200" />
        <h2>No audit entries yet</h2>
        <p>
          Every human and agent tool execution will be recorded here with
    ...
    <section className="space-y-4" aria-label="Audit history">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-2 text-lg font-semibold">Audit history</h2>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <div
            className="flex rounded-lg border border-white/8 bg-black/10 p-0.5"
            aria-label="Audit history scope"
          >
            <Button
              size="sm"
          ...
          <Input
            ...
            placeholder="Filter tool, category, or status"
            className="w-64"
      ...
      {entries.length === 0 && (
        <div className="rounded-xl border border-dashed border-white/10 p-5 text-center text-xs text-muted-foreground">
          No entries match this run and filter. Choose All history to inspect
      ...
      <div className="space-y-2">
        {entries.map((entry) => {
          ...
          return (
            <article
              key={entry.id}
              className="grid gap-3 rounded-xl border border-white/8 bg-white/2 p-3 sm:grid-cols-[32px_minmax(0,1fr)_auto]"
            >
              <div
                className={`grid size-8 place-items-center rounded-lg ${entry.success ? 'bg-emerald-400/8 text-emerald-300' : 'bg-rose-400/8 text-rose-300'}`}
              >
                {entry.success ? (
                  ...
                  onClick={() =>
                    executeTool(
                      'rollback_change',
                      {
                        ...
                        reason: `Human requested rollback from audit entry ${entry.id}.`,
                      },
                      'human-ui',
                      true,
                    )
