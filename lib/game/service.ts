...
import { forgeToolMap, ToolExecutionError, validateForgeToolInput } from '@/lib/webmcp/registry';
...
function appendActivity(state: ForgeState, toolName: string, category: string, source: string, status: ForgeState['activities'][number]['status'], summary: string) {
  state.activities.unshift({ id: nextId(state, 'activity'), toolName, category, source, status, summary, timestamp: now() });
...
export function executeForgeTool(
  ...
  if (inputErrors.length > 0) {
    ...
    const audit = appendAudit(state, { toolName, category: tool.category, source, parameters, permissionMode: state.permissionMode, approvalStatus: 'not_required', success: false, stateChanging: tool.mutatesWorld, summary: message });
    appendActivity(state, toolName, tool.category, source, 'failed', message);
    return { state, result: { ok: false, error: { code: 'INVALID_INPUT', message, details: inputErrors }, auditEntryId: audit.id, summary: message } };
  }
  ...
  if (tool.mutatesWorld && state.permissionMode === 'observe' && !options.approved) {
    ...
    appendActivity(state, toolName, tool.category, source, 'failed', 'Permission denied in OBSERVE mode.');
  ...
  if (tool.mutatesWorld && tool.requiresApproval && state.permissionMode === 'propose' && !options.approved && !options.bypassProposal) {
    ...
    appendActivity(state, toolName, tool.category, source, 'awaiting_approval', proposal.reason);
  ...
  try {
    ...
    appendActivity(state, toolName, tool.category, source, 'completed', execution.summary);
    ...
    appendActivity(state, toolName, tool.category, source, 'failed', shape.message);
...
export function rejectProposal(currentState: ForgeState, proposalId: string) {
  ...
  appendActivity(state, proposal.toolName, forgeToolMap.get(proposal.toolName)?.category ?? 'Governance', 'human-approval', 'rejected', 'Human rejected the proposed change.');
  return state;
}
...
export function hydrateForgeState(value: unknown): ForgeState {
  ...
  if (!candidate.player || !candidate.locations || !candidate.quests || !Number.isInteger(candidate.revision) || Number(candidate.revision) < 0) return createInitialForgeState();
