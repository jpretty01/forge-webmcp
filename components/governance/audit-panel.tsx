...
export function AuditPanel() {
  ...
  return (
    <section className="space-y-4" aria-label="Audit history">
      ...
      <div className="space-y-2">{entries.map((entry) => { const checkpoint = entry.checkpointId ? state.checkpoints.find((candidate) => candidate.id === entry.checkpointId) : undefined; return <article key={entry.id} className="grid gap-3 rounded-xl border border-white/8 bg-white/2 p-3 sm:grid-cols-[32px_minmax(0,1fr)_auto]"><div className={`grid size-8 place-items-center rounded-lg ${entry.success ? 'bg-emerald-400/8 text-emerald-300' : 'bg-rose-400/8 text-rose-300'}`}>{entry.success ? <CheckCircle2 className="size-4" /> : <XCircle className="size-4" />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-mono text-[11px] text-zinc-200">{entry.toolName}</p><Badge variant="outline" className="h-4 px-1 text-[8px]">{entry.category}</Badge><Badge variant="outline" className="h-4 px-1 text-[8px]">{entry.approvalStatus}</Badge></div><p className="mt-1 text-xs leading-5 text-muted-foreground">{entry.summary}</p><p className="mt-1 text-[9px] text-muted-foreground">{new Date(entry.timestamp).toLocaleString()} · {entry.source} · {entry.permissionMode}</p></div>{checkpoint && <Button size="sm" variant="outline" onClick={() => executeTool('rollback_change', { checkpoint_id: checkpoint.id, reason: `Rollback requested from audit entry ${entry.id}.` })}><RotateCcw /> Roll back</Button>}</article>; })}</div>
    </section>
  );
}
